...
import { PagedCollectionResponse } from 'src/app/component/core/models/PagedCollectionResponse';
...
export class ShopService {
  ...
  getProducts(shopParams: shopParams): Observable<PagedCollectionResponse> {
    ...
    return this.HttpClient.get<PagedCollectionResponse>(
      `${this.APIURL}/product/paginate`,
      { params: params }
    );
  }
  ...
  getRelatedProducts(categoryId: number, productId: number) {
    return this.HttpClient.get<IProduct[]>(
      `${this.APIURL}/Product/related/${categoryId}/${productId}/8`
    );
  }
}
